...
public class DashboardApiResource {
    ...
    @Autowired
    public DashboardApiResource(final PlatformSecurityContext context, final DashboardReadPlatformService dashboardReadPlatformService,
    ...
    public String generalManagerLanding(@Context final UriInfo uriInfo, @QueryParam("chartType") final String chartType,
            @QueryParam("from") final DateParam from, @QueryParam("to") final DateParam to, @QueryParam("productId") final Long productId,
            @QueryParam("officeId") final Long officeId) {
        this.context.authenticatedUser();
        JSONObject dashboardData = this.dashboardReadPlatformService.generalManagerLandingRetrieveDashboardTemplate(chartType, from, to,
                productId, officeId);
    ...
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    public String branchManagerLanding(@Context final UriInfo uriInfo, @QueryParam("chartType") final String chartType,
            @QueryParam("from") final DateParam from, @QueryParam("to") final DateParam to) {
        this.context.authenticatedUser();
    ...
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    public String managerViewDetails(@Context final UriInfo uriInfo, @QueryParam("ViewDetailsName") final String ViewDetailsName,
            @QueryParam("from") final DateParam from, @QueryParam("to") final DateParam to) {
        this.context.authenticatedUser();
    ...
    @Consumes({ MediaType.APPLICATION_JSON })
    @Produces({ MediaType.APPLICATION_JSON })
    public String loanOfficerLanding(@Context final UriInfo uriInfo, @QueryParam("chartType") final String chartType,
            @QueryParam("from") final DateParam from, @QueryParam("to") final DateParam to) {
        this.context.authenticatedUser();
        JSONObject dashboardData = this.dashboardReadPlatformService.loanOfficerLandingLandingRetrieveDashboardTemplate(chartType, from,
                to);
